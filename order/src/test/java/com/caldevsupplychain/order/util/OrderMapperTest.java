...
public class OrderMapperTest {
	...
	public void testOrderWSToOrderBean(){
		...
		/* TODO: DEBUG - this part need to be fix. Error in orderBean -> order below.
			Try to fix the bug and it happen that NPE exception has been thrown during itemBean -> iteam mapping
			Got error but not sure exactly what cause it, it seems orderMapper + itemMapper not initialize in Mapstruct
		*/
		Item item = itemMapper.toItem(itemBean, context);
		log.warn("check single Item's order currency = {}", item.getOrder().getCurrency());

		/* TODO: DEBUG - orderBean -> order mapper error */
